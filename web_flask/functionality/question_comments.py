...
@flask_app.route("/medflow/add_question_comment/<uuid:question_id>", strict_slashes=False, methods=['GET'])
def add_question_comment(question_id):
    """Renders a page for adding a question comment"""
    return render_template("question_comment.html", id=question_id, cache_id=cache_id)
...
def add_question_comment_handler(q_id):
    ...
    if auth_status[0]:
        data = request.form
        com = QueComment(question_id=question.id, user_id=user_id, **data)
        storage.add(com)
        storage.save()
...
def update_comment(q_id, c_id):
    ...
    return render_template("update_qcomment.html", q_id=q_id, c_id=c_id, cache_id=cache_id)
